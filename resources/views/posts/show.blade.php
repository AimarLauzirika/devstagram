@extends('layouts.app')

@section('titulo')
    {{$post->titulo}}
@endsection

@section('contenido')
    <div class="container mx-auto md:flex">
        <div class="md:w-1/2">
            <img src="{{asset('uploads') . '/' . $post->imagen}}" alt="Imagen del post">
            <div class="p-3 items-center">
                @auth
                    {{-- {{dd($post->likes->count())}} --}}
                    @if ($post->checkLike(auth()->user()))
                    <form action="{{route('posts.likes.destroy', $post)}}" method="POST">
                        @method('DELETE')
                        @csrf
                        <div class="my-4">
                            <button type="submit" class="flex gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill= "red" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                                </svg>
                                <p>{{$post->likes->count()}} Likes</p>
                            </button>
                        </div>
                    </form>
                    @else
                        @if (auth()->user()->id === $post->user_id)
                        <p>Este es mi post, no le puedo dar Like {{$post->likes->count()}} Likes</p>
                        @else
                            <form action="{{route('posts.likes.store', $post)}}" method="POST">
                                @csrf
                                <div class="my-4">
                                    <button type="submit" class="flex gap-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill= "white" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                                        </svg>
                                        <p>{{$post->likes->count()}} Likes</p>
                                    </button>
                                </div>
                            </form>
                        @endif
                    @endif
                @endauth
                @guest
                <p>{{$post->likes->count()}} Likes</p>
                @endguest
            </div>
            <div>
                <p class="font-bold">{{$post->user->username}}</p>
                <p class="text-sm text-gray-500">{{$post->created_at->diffForHumans()}}</p>
                <p class="mt-5">{{$post->descripcion}}</p>
            </div>

            {{-- {{dd($user->username)}} --}}
            @auth
            @if (auth()->user()->id === $post->user_id)
                <form action="{{route('posts.destroy', $post)}}" method="POST">
                    @csrf
                    @method('delete')
                    <input type="submit" value="Eliminar Post" class="bg-red-500 hover:bg-red-600 p-2 rounded text-white font-bold mt-4 cursor-pointer">
                </form>
            @endif
            @endauth
        </div>
        <div class="md:w-1/2 p-5">
            <div class="shadow bg-white p-5 mb-5">

                @auth
                    
                {{-- <p class="text-xl font-bold text-center mb-4">Agregar un nuevo Comentario</p> --}}

                @if (session('mensaje'))
                    <div class="bg-green-500 p-2 rounded-lg mb-6 text-white text-center uppercase font-bold">
                        {{session('mensaje')}}
                    </div>
                @endif
                <form action="{{route('comentarios.store', [$user, $post])}}" method="POST">
                    @csrf
                    <div class="mb-5">
                        <label class="mb-2 block">
                            <p class="uppercase text-gray-500 font-bold pl-2">Añade un Comentario</p>
                            <textarea
                            id="comentario"
                            name="comentario"
                            placeholder="Escribe aquí tu comentario..."
                            class="border p-3 w-full rounded-lg font-normal @error('comentario') border-red-500 @enderror"
                            >{{old('comentario')}}</textarea>
                        </label>
                        @error('comentario')
                        <p class="text-red-500 py-0 rounded-lg text-xs p-2 text-center">{{$message}}</p>
                        @enderror
                    </div>
                    <input type="submit" value="Comentar" class="bg-sky-600 hover:bg-sky-700 transition-colors cursor-pointer uppercase font-bold w-full p-3 text-white rounded-lg">
                </form>
                @endauth

                <div class="bg-white shadow mb-5 max-h-96 overflow-y-auto mt-10">
                    @if ($post->comentarios->count())
                        @foreach ($post->comentarios as $comentario)
                            <div class="p-5 border-gray-300 border-b">
                                <a href="{{route('posts.index', $comentario->user)}}" class="text-sm text-gray-500 font-semibold">{{$comentario->user->username}}</a>
                                <pre><span class="font-sans">{{$comentario->comentario}}</span></pre>
                                <p class="text-xs text-gray-500">{{$comentario->created_at->diffForHumans()}}</p>
                            </div>
                        @endforeach
                    @else
                        <p class="p-10 text-center">No Hay Comentarios Aún</p>
                    @endif
                </div>
            </div>
        </div>
    </div>
    @endsection